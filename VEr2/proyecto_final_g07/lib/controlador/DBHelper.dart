...
class DBHelper {
  ...
  _onCreate(Database db, int version) async {
    await db.execute(
        'CREATE TABLE student (id INTEGER PRIMARY KEY, name TEXT, apellido TEXT, cedula TEXT, correo TEXT, direccion TEXT, foto TEXT, sexo TEXT, fecha TEXT, beca TEXT, materias TEXT, user TEXT, password TEXT)');
  }
  ...
  Future<int> newClient(User newClient) async {
    ...
    var res = await dbClient.rawInsert(
        "INSERT Into student (id,name,apellido,cedula,correo,direccion,foto,sexo,fecha,beca,materias,user,password) VALUES (((SELECT MAX(id) FROM student)+1),\'${newClient.name}\', \'${newClient.apellido}\',\'${newClient.cedula}\',\'${newClient.correo}\',\'${newClient.direccion}\',\'${newClient.foto}\',\'${newClient.sexo}\',\'${newClient.fecha}\',\'${newClient.beca}\',\'${newClient.materias}\',\'${newClient.user}\',\'${newClient.password}\')");
    return res;
  }

  //UPDATE public.singer
  //   SET id=?, first_name=?, last_name=?, birth_date=?
  // WHERE <condition>;
  Future<int> updateClient(User newClient, int id) async {
    final dbClient = await db;
    var res = await dbClient.rawInsert(
        "UPDATE student SET name = \'${newClient.name}\', apellido = \'${newClient.apellido}\', cedula = \'${newClient.cedula}\', correo = \'${newClient.correo}\', direccion = \'${newClient.direccion}\', foto = \'${newClient.foto}\', sexo = \'${newClient.sexo}\', fecha = \'${newClient.fecha}\', beca = \'${newClient.beca}\', materias = \'${newClient.materias}\', user = \'${newClient.user}\', password = \'${newClient.password}\' WHERE id = ${id};");
    return res;
  }
  ...
  Future<List<User>> buscarUnDato(String user, String pass) async {
    ...
    List<Map> list = await db_connection.rawQuery(
        'SELECT * FROM student WHERE name = \'${user}\' AND apellido = \'${pass}\'');
  ...
  Future<List<User>> getUserTodos() async {
    var dbClient = await db;
    List<User> usersList = List();
    List<Map> queryList = await dbClient.rawQuery(
      'SELECT * FROM student',
  ...
  Future<User> getUser2(String email, String password) async {
    ...
    if (queryList != null && queryList.length > 0) {
      ...
      User us = new User(
          usersList[0].id,
          usersList[0].name,
          usersList[0].apellido,
          usersList[0].cedula,
          usersList[0].correo,
          usersList[0].direccion,
          usersList[0].foto,
          usersList[0].sexo,
          usersList[0].fecha,
          usersList[0].beca,
          usersList[0].materias,
          usersList[0].user,
